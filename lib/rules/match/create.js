...
function validateFolders(options, parsedPath) {
   ...
   */
  const errors = [];
  const { match, ignoreSegments } = options;
  ...
  parsedPath.dir.split(path.sep).forEach((directory) => {
    ...
    const ignoreSegment = ignoreSegments.some((regex) => regex.test(directory));
    if (ignoreSegment) {
      return;
    }
...
function validateFilename(options, parsedPath) {
  ...
  let parsedPathName = parsedPath.name;

  const ignoreSegment = ignoreSegments.some((regex) =>
    regex.test(parsedPathName)
  );
  if (ignoreSegment) {
    return [];
  }
  ...
  // Issue #2 fix
  const testFilePostfixes = [".test", ".spec"];
  for (const testFilePostfix of testFilePostfixes) {
    if (parsedPathName.endsWith(testFilePostfix)) {
      parsedPathName = parsedPathName.replace(testFilePostfix, "");
    }
  }

  const fileMatchesRegex = match.some((regex) => regex.test(parsedPathName));
  const errors = [];
